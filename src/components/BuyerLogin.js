...
const BuyerLogin = () => {
  ...
  // Animation classes
  const cardBase = 'w-full max-w-md bg-white/80 backdrop-blur-md rounded-2xl shadow-2xl p-8 border border-green-200 transition-all duration-500';
  ...
  // Placeholder handlers (no backend yet)
  const handleLogin = async (e) => {
    ...
    // 2. Check if user exists in buyers table
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-100 via-lime-200 to-green-300 p-4 overflow-hidden">
      <div className="relative w-full max-w-md h-[500px]">
        ...
        <div className={`${cardBase} ${cardLogin}`}
          ...
          <h2 className="text-3xl font-bold text-green-800 text-center mb-6 flex items-center justify-center gap-2">
            <User className="w-7 h-7 text-green-600" /> Buyer Login
          </h2>
        ...
        <div className={`${cardBase} ${cardSignup}`}
             style={{ boxShadow: '0 8px 32px 0 rgba(34,197,94,0.15)' }}>
          <h2 className="text-3xl font-bold text-green-800 text-center mb-6 flex items-center justify-center gap-2">
            <User className="w-7 h-7 text-green-600" /> Buyer Sign-Up
          </h2>
          ...
          <div className="mt-4 text-center text-sm text-green-600">
            ...
            >
              <ArrowLeftCircle className="w-4 h-4 inline" /> Back to Login
            </span>
          </div>
          {error && isSignup && <div className="text-red-600 text-center mt-2 text-sm">{error}</div>}
          {success && isSignup && <div className="text-green-600 text-center mt-2 text-sm">{success}</div>}
      ...
      <style>{`
        ...
        .transition-all { transition: all 0.5s cubic-bezier(.4,2,.6,1); }
      `}</style>
    </div>
  );
};
